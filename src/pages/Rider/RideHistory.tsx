...
const RideHistory = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState<RideStatus | 'ALL'>('ALL')
  ...
  const filteredRides = selectedStatus === 'ALL' ? rides : rides.filter(r => r.status === selectedStatus)

  // Pagination calculation
  const totalPages = Math.ceil(filteredRides.length / itemsPerPage)
  const paginatedRides = filteredRides.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-pink-50 to-yellow-50 px-4 py-10">
      <h1 className="text-3xl font-extrabold mb-6 text-center text-indigo-700">My Rides</h1>

      {/* Filter */}
      <div className="flex justify-center mb-6 gap-3 flex-wrap">
        {(['ALL', 'REQUESTED', 'ACCEPTED', 'PICKED_UP', 'COMPLETED', 'CANCELLED'] as (RideStatus | 'ALL')[]).map((status) => (
          <button
            key={status}
      ...
      <div className="grid gap-6 max-w-5xl mx-auto">
        {paginatedRides.length === 0 && <div className="text-center text-gray-500">No rides found.</div>}
        ...
        <div className="flex justify-center gap-2 mt-6 flex-wrap">
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`px-3 py-1 rounded ${currentPage === i + 1 ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-300 hover:bg-indigo-100'}`}
            >
              {i + 1}
            </button>
          ))}
