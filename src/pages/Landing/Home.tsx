...
export default function Home() {
  const { data, isLoading, isError } = useGetAllRidesPublicQuery()
  const rides: Ride[] = data?.data || []

  // Sort rides by createdAt descending
  const sortedRides = rides
    .filter(r => r.createdAt)
    .sort((a, b) => new Date(b.createdAt!).getTime() - new Date(a.createdAt!).getTime())
    .slice(0, 6) // Show only the 6 most recent rides
  ...
  return (
    <div className="min-h-screen px-4 py-10 bg-gradient-to-br from-purple-50 via-pink-50 to-yellow-50 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto space-y-20 relative z-10">
        ...
        {/* Recent Rides */}
        <section>
          <motion.h3 className="font-bold text-3xl mb-6 text-indigo-700" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
            Recent Requests (Public)
          </motion.h3>

          {isLoading && <p className="text-gray-500">Loading rides...</p>}
          {isError && <p className="text-red-500">Failed to load rides.</p>}
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {sortedRides.map((r) => (
                ...
                >
                  ...
                  <Link to={`/rides/${r._id}`} className="text-indigo-600 mt-2 block hover:underline">View Details</Link>
                </motion.div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500">No recent ride requests available.</p>
          )}
        </section>
